...
void loop()
{
  ...
  if (lightOn) {
    ...
    bluetooth.print("Difference: ");
    bluetooth.println(newTemp - baselineTemp);
  }
...
void rotateMarshmallow() {
  ...
  } while ((double(minute2) + (double(second2) / 60.0)) - (double(minute) + (double(second) / 60.0)) < 1.25);
  // displayTime();
  Wire.endTransmission();
